...
type Props = {
    data: PostsListSectionData;
    posts: Post[];
};

export default function PostsListSectionView({ data, posts }: Props) {
    const heading = data.heading?.trim() || "Posts";

    return (
        <section className="grid gap-6">
            <h2
                className="heading-rule m-0 font-bold text-[var(--page-h1-color,var(--color-chestnut))] dark:text-[var(--page-h1-color-dark,var(--color-dark-text))]"
            ...
            {posts.length === 0 ? (
                <div className="surface-card text-center">
                    <p
                        className="text-[var(--page-body-color,var(--color-olive-dark))] dark:text-[var(--page-body-color-dark,var(--color-dark-muted))]"
                ...
                <div className="grid gap-4 sm:gap-6 lg:grid-cols-2">
                    {posts.map((post) => (
                        ...
                        >
                            ...
                            <h3
                                className="text-xl font-semibold leading-tight text-[var(--page-h2-color,var(--color-chestnut))] dark:text-[var(--page-h2-color-dark,var(--color-dark-text))]"
                                style={{ fontFamily: "var(--page-heading-font, var(--font-display))" }}
                            >
                                {post.title}
                            </h3>
                            <p
                                className="mt-3 line-clamp-3 text-[0.98rem] leading-relaxed text-[var(--page-body-color,var(--color-chestnut-dark))] dark:text-[var(--page-body-color-dark,var(--color-dark-muted))]"
                                style={{ fontFamily: "var(--page-body-font, inherit)" }}
                            >
                                {post.summary}
                            </p>
