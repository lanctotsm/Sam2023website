"use client";

import type { PostsPageSettings, PostsPageTemplateId } from "@/lib/postsPage";
import {
    createPostsPageSection,
    DEFAULT_POSTS_LIST_SECTION_ID,
    getPostsPageSectionDisplayLabel,
} from "@/lib/postsPage";
import {
    getAddTemplateLabel,
    getPostsPageRegistryEntry,
    POSTS_PAGE_ADD_TEMPLATE_IDS,
} from "@/components/posts/sectionRegistry";
import { createHomeSection } from "@/lib/frontPageDefaults";

type Props = {
    config: PostsPageSettings;
    setConfig: React.Dispatch<React.SetStateAction<PostsPageSettings>>;
    sectionClass: string;
    inputClass: string;
    btnAdd: string;
    btnDanger: string;
};

export default function PostsPageSectionOrderPanel({
    config,
    setConfig,
    sectionClass,
    inputClass,
    btnAdd,
    btnDanger,
}: Props) {
    const setSectionOrder = (order: string[]) => setConfig((c) => ({ ...c, sectionOrder: order }));

    const removeFromPage = (id: string) => {
        if (id === DEFAULT_POSTS_LIST_SECTION_ID) return;
        setSectionOrder(config.sectionOrder.filter((k) => k !== id));
    };

    const moveSection = (index: number, direction: -1 | 1) => {
        const next = [...config.sectionOrder];
        const target = index + direction;
        if (target < 0 || target >= next.length) return;
        [next[index], next[target]] = [next[target], next[index]];
        setSectionOrder(next);
    };

    const addSectionToPage = (id: string) => {
        if (config.sectionOrder.includes(id)) return;
        setSectionOrder([...config.sectionOrder, id]);
    };

    const addNewSection = (templateId: PostsPageTemplateId) => {
        if (templateId === "posts-list") return;
        const section = createPostsPageSection(templateId);
        setConfig((c) => ({
            ...c,
            sections: [...c.sections, section],
            sectionOrder: [...c.sectionOrder, section.id],
        }));
    };

    const addTextBlock = () => {
        const homeSection = createHomeSection("text-block");
        const section = {
            id: homeSection.id,
            templateId: "text-block" as const,
            label: homeSection.label,
            removable: true,
            data: homeSection.data,
        };
        setConfig((c) => ({
            ...c,
            sections: [...c.sections, section],
            sectionOrder: [...c.sectionOrder, section.id],
        }));
    };

    const hiddenSections = config.sections.filter((s) => !config.sectionOrder.includes(s.id));

    return (
        <section className={sectionClass}>
            <h2 className="mb-2 text-lg font-semibold text-chestnut dark:text-dark-text">Posts page sections</h2>
            <p className="mb-4 text-sm text-olive-dark dark:text-dark-muted">
                Reorder sections on /posts. The Posts list section cannot be removed — use the toggle above to hide the
                blog grid.
            </p>
            {config.sectionOrder.length === 0 ? (
                <p className="text-sm text-olive dark:text-dark-muted">No sections on the page. Add one below.</p>
            ) : (
                <ul className="mb-4 space-y-2">
                    {config.sectionOrder.map((id, index) => {
                        const section = config.sections.find((s) => s.id === id);
                        if (!section) return null;
                        const template = getPostsPageRegistryEntry(section.templateId);
                        const isLocked = id === DEFAULT_POSTS_LIST_SECTION_ID || section.templateId === "posts-list";
                        return (
                            <li
                                key={id}
                                className="flex flex-wrap items-center gap-2 rounded-lg border border-desert-tan-dark/50 bg-white/60 px-3 py-2 dark:border-dark-muted/50 dark:bg-dark-bg/40"
                            >
                                <span className="min-w-0 flex-1 text-sm font-medium text-chestnut-dark dark:text-dark-text">
                                    {getPostsPageSectionDisplayLabel(section)}
                                    <span className="ml-2 text-xs font-normal text-olive dark:text-dark-muted">
                                        ({template?.label ?? section.templateId})
                                    </span>
                                </span>
                                <button
                                    type="button"
                                    disabled={index === 0}
                                    onClick={() => moveSection(index, -1)}
                                    className={btnAdd}
                                    aria-label="Move up"
                                >
                                    ↑
                                </button>
                                <button
                                    type="button"
                                    disabled={index === config.sectionOrder.length - 1}
                                    onClick={() => moveSection(index, 1)}
                                    className={btnAdd}
                                    aria-label="Move down"
                                >
                                    ↓
                                </button>
                                {!isLocked && (
                                    <button type="button" onClick={() => removeFromPage(id)} className={btnDanger}>
                                        Remove
                                    </button>
                                )}
                            </li>
                        );
                    })}
                </ul>
            )}
            <div className="flex flex-wrap items-center gap-2">
                {hiddenSections.length > 0 && (
                    <select
                        className={inputClass}
                        defaultValue=""
                        onChange={(e) => {
                            const id = e.target.value;
                            if (id) {
                                addSectionToPage(id);
                                e.target.value = "";
                            }
                        }}
                    >
                        <option value="" disabled>
                            Add existing section…
                        </option>
                        {hiddenSections.map((section) => (
                            <option key={section.id} value={section.id}>
                                {getPostsPageSectionDisplayLabel(section)}
                            </option>
                        ))}
                    </select>
                )}
                <select
                    className={inputClass}
                    defaultValue=""
                    onChange={(e) => {
                        const templateId = e.target.value as PostsPageTemplateId;
                        if (templateId) {
                            addNewSection(templateId);
                            e.target.value = "";
                        }
                    }}
                >
                    <option value="" disabled>
                        Add from template…
                    </option>
                    {POSTS_PAGE_ADD_TEMPLATE_IDS.map((templateId) => (
                        <option key={templateId} value={templateId}>
                            {getAddTemplateLabel(templateId)}
                        </option>
                    ))}
                </select>
                <button type="button" onClick={addTextBlock} className={btnAdd}>
                    + Text block
                </button>
            </div>
        </section>
    );
}
