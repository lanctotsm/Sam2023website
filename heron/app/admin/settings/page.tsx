            ...
            {activeTab === "homepage" && (
                <>
                                ...
                                <div>
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex flex-col items-center justify-center rounded-lg border-2 border-dashed border-desert-tan-dark bg-white/50 p-6 text-center transition-colors hover:bg-white dark:border-dark-muted dark:bg-dark-bg/50 dark:hover:bg-dark-surface">
                                            ...
                                            <p className="mb-2 text-sm text-chestnut-dark dark:text-dark-muted">Drag & drop an image here, or click to select</p>
                                            <input
                                                type="file"
                                            ...
                                            />
                                            <button type="button" onClick={() => document.getElementById("bg-image-upload")?.click()} className={btnAdd}>
                                                Select Image
                                            </button>
                                        </div>
                                        <div className="flex items-center gap-2">
                                        ...
                                        {config.hero.backgroundImage && (
                                            <div className="mt-2 text-sm text-olive-dark dark:text-dark-muted">
                                                ...
                                                <img src={config.hero.backgroundImage} alt="Background preview" className="mt-2 max-h-48 w-full rounded-lg border border-desert-tan-dark object-cover dark:border-dark-muted" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} onLoad={(e) => { (e.target as HTMLImageElement).style.display = "block"; }} />
                                            </div>
                                        )}
                                    </div>
                                </div>
                            ...
                            {config.about.paragraphs.map((p, i) => (
                                <div key={i} className="relative">
                                    ...
                                    {config.about.paragraphs.length > 1 && (
                                        <button type="button" onClick={() => updateAbout({ paragraphs: config.about.paragraphs.filter((_, j) => j !== i) })} className={`mt-1 ${btnDanger}`}>Remove</button>
                                    )}
                                </div>
                            ))}
                            <button type="button" onClick={() => updateAbout({ paragraphs: [...config.about.paragraphs, ""] })} className={btnAdd}>+ Add Paragraph</button>
                        </div>
                    </section>
                    ...
                    <section className={sectionClass}>
                        ...
                        <div className="grid gap-4">
                            ...
                            <div>
                                ...
                                <input type="range" min={1} max={4} value={config.cards.columns} onChange={(e) => updateCards({ columns: Number(e.target.value) })} className="w-full accent-chestnut dark:accent-caramel" />
                                <div className="flex justify-between text-xs text-olive dark:text-dark-muted"><span>1</span><span>2</span><span>3</span><span>4</span></div>
                            </div>
                            {config.cards.items.map((item, i) => (
                                <div key={i} className="rounded-lg border border-desert-tan-dark/50 bg-white/50 p-4 dark:border-dark-muted/50 dark:bg-dark-bg/50">
                                    ...
                                    <div className="grid gap-3">
                                        ...
                                        <div><label className={labelClass}>Title</label><input className={inputClass} value={item.title} onChange={(e) => updateCardItem(i, { title: e.target.value })} /></div>
                                        <div><label className={labelClass}>Description</label><textarea className={textareaClass} value={item.text} onChange={(e) => updateCardItem(i, { text: e.target.value })} /></div>
                                    </div>
                                </div>
                            ))}
                    ...
                    <section className={sectionClass}>
                        ...
                        <div className="grid gap-4">
                            ...
                            {config.journey.paragraphs.map((p, i) => (
                                <div key={i}>
                                    <label className={labelClass}>Paragraph {i + 1}</label>
                                    <textarea className={textareaClass} value={p} onChange={(e) => { const newParagraphs = [...config.journey.paragraphs]; newParagraphs[i] = e.target.value; updateJourney({ paragraphs: newParagraphs }); }} />
                                    {config.journey.paragraphs.length > 1 && (
                                        <button type="button" onClick={() => updateJourney({ paragraphs: config.journey.paragraphs.filter((_, j) => j !== i) })} className={`mt-1 ${btnDanger}`}>Remove</button>
                                    )}
                                </div>
                            ))}
                            <button type="button" onClick={() => updateJourney({ paragraphs: [...config.journey.paragraphs, ""] })} className={btnAdd}>+ Add Paragraph</button>
                        </div>
                    </section>
                    ...
                    <section className={sectionClass}>
                        ...
                        <div className="grid gap-4">
                            <div><label className={labelClass}>Section Heading</label><input className={inputClass} value={config.interests.heading} onChange={(e) => updateInterests({ heading: e.target.value })} /></div>
                            {config.interests.items.map((item, i) => (
                                <div key={i} className="flex items-center gap-3">
                                    <IconPicker value={item.icon} onChange={(icon) => updateInterestItem(i, { icon })} />
                                    <input className={inputClass} value={item.label} onChange={(e) => updateInterestItem(i, { label: e.target.value })} placeholder="Label" />
                    ...
                    <section className={sectionClass}>
                        ...
                        <div className="grid gap-4">
                            <div><label className={labelClass}>Section Heading</label><input className={inputClass} value={config.contact.heading} onChange={(e) => updateContact({ heading: e.target.value })} /></div>
                            <div><label className={labelClass}>Description Text</label><textarea className={textareaClass} value={config.contact.text} onChange={(e) => updateContact({ text: e.target.value })} /></div>
                            <div className="flex items-center gap-3">
                                    ...
                                    {config.contact.links.map((link, i) => (
                                        <div key={i} className="rounded-lg border border-desert-tan-dark/50 bg-white/50 p-4 dark:border-dark-muted/50 dark:bg-dark-bg/50">
                                            ...
                                            <div className="grid gap-3">
                                                ...
                                                <div><label className={labelClass}>Label</label><input className={inputClass} value={link.label} onChange={(e) => updateContactLink(i, { label: e.target.value })} /></div>
                                                <div><label className={labelClass}>URL</label><input className={inputClass} value={link.url} onChange={(e) => updateContactLink(i, { url: e.target.value })} placeholder="https://… or mailto:…" /></div>
                                            </div>
                                        </div>
                                    ))}
